...
public class ProfileController {
	...
	@Inject
	public ProfileController(ScooldUtils utils, GravatarAvatarGenerator gravatarAvatarGenerator, AvatarProviderProxy avatarRepository, AvatarConfig avatarConfig) {
		this.utils = utils;
	...
	public String get(@PathVariable(required = false) String id, HttpServletRequest req, Model model) {
		...
		model.addAttribute("ogimage", avatarProvider.getLink(showUser, AvatarFormat.Profile));
	...
	private boolean updateUserPictureAndName(Profile showUser, String picture, String name) {
		...
		if (Config.getConfigBoolean("avatar_edits_enabled", true) &&
			...
			AvatarStorageResult result = changeAvatarInProfile(showUser, picture);
	...
	private AvatarStorageResult changeAvatarInProfile(Profile profile, String url) {
